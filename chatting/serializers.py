...
class ChatRoomSerializer(serializers.ModelSerializer):
    ...
    mentee_id = serializers.IntegerField(source='mentor.user.id', read_only=True)
    chats = ChatSerializer(many=True, read_only=True, source='chat_set')
    last_chat = serializers.SerializerMethodField()

    class Meta:
        model = Chatroom
        fields = ['id', 'interests', 'title', 'mentor_response', 'mentor', 'last_chat', 'mentor_name', 'mentee', 'mentee_id', 'mentee_name', 'chats']
